...
class ContinuousDoorOpener:
    ...
    def compute_combined_motion(self, force_state: ForceState) -> Tuple[np.ndarray, float]:
        ...
        if lateral_force > 50:
            max_rotation = min(
                self.max_rotation_per_step * 0.4, 0.08
            )  # Max 4.6 degrees at very high force
        elif lateral_force > 30:
            max_rotation = min(
                self.max_rotation_per_step * 0.6, 0.12
            )  # Max 6.9 degrees at high force
        else:
    ...
    def setup_simulation(self):
        ...
        # Reset camera
        self.meshcat.SetCameraPose(camera_in_world=[1.5, 1.5, 1.2], target_in_world=[0.0, 0.0, 0.3])
    ...
    def create_visualizations(self):
        ...
        # Force vector (red)
        self.meshcat.SetObject("force_vector/arrow", Cylinder(0.005, 0.1), Rgba(1.0, 0.0, 0.0, 0.8))
        self.meshcat.SetObject("force_vector/head", Sphere(0.015), Rgba(1.0, 0.0, 0.0, 0.9))

        # Motion direction (green)
        self.meshcat.SetObject(
            "motion_direction/arrow", Cylinder(0.006, 0.1), Rgba(0.0, 1.0, 0.0, 0.8)
        )
        self.meshcat.SetObject("motion_direction/head", Sphere(0.018), Rgba(0.0, 1.0, 0.0, 0.9))

        # Pivot point (yellow)
        self.meshcat.SetObject("pivot_point", Sphere(0.02), Rgba(1.0, 1.0, 0.0, 0.8))

        # Pivot axis (vertical line)
        self.meshcat.SetObject("pivot_axis", Cylinder(0.003, 0.3), Rgba(1.0, 1.0, 0.0, 0.5))
    ...
    def update_visualizations(
        self, force_state: ForceState, translation: np.ndarray, rotation_angle: float
    ):
        ...
        self.meshcat.SetTransform(
            "pivot_point", RigidTransform(RotationMatrix(), pivot_point_world)
        )
        self.meshcat.SetTransform("pivot_axis", RigidTransform(RotationMatrix(), pivot_point_world))
        ...
        if force_magnitude > 0.1:
            ...
            self.meshcat.SetTransform("force_vector/arrow", RigidTransform(rotation, arrow_center))

            head_position = openft_pos + force_direction * force_length
            self.meshcat.SetTransform(
                "force_vector/head", RigidTransform(RotationMatrix(), head_position)
            )
        ...
        if np.linalg.norm(translation) > 0.001:
            ...
            self.meshcat.SetTransform(
                "motion_direction/arrow", RigidTransform(rotation, arrow_center)
            )

            head_position = openft_pos + motion_unit * motion_length
            self.meshcat.SetTransform(
                "motion_direction/head", RigidTransform(RotationMatrix(), head_position)
            )
    ...
    def run(self):
            ...
            while True:
                ...
                if force_state is not None:
                    ...
                    if self.end_angle is not None and abs(self.total_rotation) >= self.end_angle:
                        print(f"\n{'=' * 60}")
                        print(
                            f"Target angle reached! Total rotation: {np.degrees(abs(self.total_rotation)):.1f}° >= {np.degrees(self.end_angle):.1f}°"
                        )
        ...
        print(
            f"  Average rate: {self.motion_count / (time.time() - self.last_motion_time + 0.001):.1f} Hz"
        )
        print(f"{'=' * 60}")
...
def main():
    ...
    parser.add_argument("--xarm", type=str, help="xARM IP address for robot control")
    parser.add_argument(
        "--sim_only", action="store_true", help="Run in simulation only (don't command real robot)"
    )
    parser.add_argument(
        "--pivot_distance",
        type=float,
    ...
    parser.add_argument(
        "--force_threshold", type=float, default=10.0, help="Target force threshold (N)"
    )
    parser.add_argument(
        "--rotation_gain",
        type=float,
    ...
    parser.add_argument(
        "--pull_speed", type=float, default=0.015, help="Pull distance per step (meters)"
    )
    parser.add_argument(
        "--sensor_delay", type=float, default=0.2, help="Expected sensor delay (seconds)"
    )
    parser.add_argument(
        "--prediction_gain",
        type=float,
